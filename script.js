	...
	let isiValue = [];
	let iniPrima = bilanganPrima(1000);
	function isPresent(x, y){
	    for(let i=0; i<isiValue.length; i++){
	        if(isiValue[i].x == x && isiValue[i].y == y){
	            return true;
	        }
	    }
	    return false;
	...
	function dinding(level){
	    ...
	    if(level==2){
			...
			let dindingLevelDua1 = addWall(5, HEIGHT-3, WIDTH - 10, 1);
	        positions  = dindingLevelDua1;
	    }else if(level==3){
			let dindingTiga1 = addWall(20, HEIGHT/2, WIDTH - 15, 1);
	    	let dindingTiga2 = addWall(5, HEIGHT-3, WIDTH - 10, 1);
	        positions  = dindingTiga1.concat(dindingTiga2);
	    }else if(level==4){
			
			let dindingLevelEmpat1 = addWall(5, HEIGHT-3, WIDTH - 10, 1);
			let dindingEmpat2 = addWall(20, HEIGHT/2, WIDTH - 15, 1);
	    	let dindingEmpat3 = addWall(5, 2, 1, HEIGHT-10);
	        positions  = dindingLevelEmpat1.concat(dindingEmpat2,dindingEmpat3);
	    }else if(level>=5){
			let dindingLevelLima1 = addWall(10, HEIGHT-27, WIDTH - 15, 1);
			let dindingLevelLima2 = addWall(5, HEIGHT-3, WIDTH - 10, 1);
			let dindingLevelLima3 = addWall(20, HEIGHT/2, WIDTH - 15, 1);
			let dindingLevelLima4 =  addWall(5, 2, 1, HEIGHT-10);
